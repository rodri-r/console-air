...
describe(TransactionMessageData.name, () => {
  describe("getCreateDeploymentMsg", () => {
    it("forwards the reclamation field into the message when present", () => {
      const reclamation = DeploymentReclamation.fromPartial({ minWindow: { seconds: 86400 } });
      const { deploymentData } = setup({ reclamation });

      const result = TransactionMessageData.getCreateDeploymentMsg(deploymentData);

      expect(result.value.reclamation).toEqual(reclamation);
    });
    ...
    it("leaves reclamation unset when the deployment has none", () => {
      ...
      expect(result.value.reclamation).toBeUndefined();
    });

    function setup(input: { reclamation?: DeploymentReclamation }) {
      const deploymentData: NewDeploymentData = {
        sdl: {},
        manifest: [],
        groups: [],
        deploymentId: { owner: "akash1abc", dseq: "12345" },
        orderId: [],
        leaseId: [],
